...
use glib::{ffi::GBytes, object::IsA, translate::*};

use crate::{Context, Value, ValueExt};
...
impl<O:IsA<Value>> ValueExtManual for O {
	...
	fn array_buffer_get_data(&self) -> &[u8] {
		unsafe {
			let mut len = 0;
			let ptr = ffi::jsc_value_array_buffer_get_data(
	...
	fn typed_array_get_data(&self) -> TypedArrayData {
		...
		unsafe {
			let mut len = 0;
			let ptr = ffi::jsc_value_typed_array_get_data(
				self.as_ref().to_glib_none().0,
				&mut len,
			);
			if ptr.is_null() || len == 0 {
				...
				match self.typed_array_get_type() {
					...
					Int8 => {
						TypedArrayData::Int8(slice::from_raw_parts(
							ptr as *const i8,
							len,
						))
					},
					Int16 => {
						TypedArrayData::Int16(slice::from_raw_parts(
							ptr as *const i16,
							len,
						))
					},
					Int32 => {
						TypedArrayData::Int32(slice::from_raw_parts(
							ptr as *const i32,
							len,
						))
					},
					Int64 => {
						TypedArrayData::Int64(slice::from_raw_parts(
							ptr as *const i64,
							len,
						))
					},
					Uint8 => {
						TypedArrayData::Uint8(slice::from_raw_parts(
							ptr as *const u8,
							len,
						))
					},
					Uint8Clamped => {
						TypedArrayData::Uint8Clamped(slice::from_raw_parts(
							ptr as *const u8,
							len,
						))
					},
					Uint16 => {
						TypedArrayData::Uint16(slice::from_raw_parts(
							ptr as *const u16,
							len,
						))
					},
					Uint32 => {
						TypedArrayData::Uint32(slice::from_raw_parts(
							ptr as *const u32,
							len,
						))
					},
					Uint64 => {
						TypedArrayData::Uint64(slice::from_raw_parts(
							ptr as *const u64,
							len,
						))
					},
					Float32 => {
						TypedArrayData::Float32(slice::from_raw_parts(
							ptr as *const f32,
							len,
						))
					},
					Float64 => {
						TypedArrayData::Float64(slice::from_raw_parts(
							ptr as *const f64,
							len,
						))
					},
